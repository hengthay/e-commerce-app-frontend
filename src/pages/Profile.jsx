...
const Profile = () => {
  ...
  const phone = profileData.phone_number ? (profileData.phone_number.startsWith("+") ? profileData.phone_number : `+855 ${profileData.phone_number}`) : "";
  const street = profileData.street || "";
  const city = profileData.city || "";
  const country = profileData.country || "";

  const hasAddress = Boolean(street || city || country || profileData.phone_number);
  ...
  return (
    <section className="section-container max-w-5xl max-sm:w-[350px] mx-auto md:px-6 py-8 lg:py-12 flex flex-col gap-8 md:my-10 my-14">
      <div className="w-full mt-10 shadow bg-white lg:p-8 p-6 rounded-md">
        <div className="w-full flex flex-col justify-center items-center gap-y-6">
          <div className="my-4 w-full space-y-4">
            <h3 className="text-3xl sm:text-2xl lg:text-3xl font-extrabold leading-tight text-gray-800">
              My Profile
            </h3>
            ...
            <div className="w-full flex flex-col md:flex-row justify-between shadow py-6 px-6 md:px-8 rounded-lg gap-6">
              ...
              <div className="flex md:flex-row flex-col sm:items-center items-start gap-x-6 w-full md:w-auto space-y-2">
                <img
                  src={profileData.avatar || "https://png.pngtree.com/png-clipart/20230927/original/pngtree-man-avatar-image-for-profile-png-image_13001877.png"}
                  alt="Avatar-User"
              ...
              <div className="flex justify-start items-center md:items-start sm:mt-2">
                ...
                >
                  Edit <CiEdit size={22} className="font-semibold text-gray-700" />
                </Link>
              </div>
            </div>
            ...
            <div className="w-full flex flex-col shadow py-6 px-6 md:px-8 rounded-lg space-y-6">
              <div className="w-full flex justify-between items-center">
                <h3 className="md:text-2xl text-xl font-semibold text-gray-800">Personal Information</h3>
                <Link to="/profile/edit" className="flex items-center gap-x-1.5 text-base text-gray-600 border border-gray-300 px-3 py-1.5 rounded-4xl hover:shadow-sm hover:bg-gray-50 transition-all">
                  Edit <CiEdit size={22} className="font-semibold text-gray-700" />
                </Link>
              </div>
              ...
              <div className="grid md:grid-cols-2 grid-cols-1 gap-6">
                ...
                <div>
                  ...
                  <p className="text-base font-medium text-gray-800">{phone || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Role</p>
                  <p className="text-base font-medium text-gray-800">{role || "User"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="w-full flex flex-col shadow py-6 px-6 md:px-8 rounded-lg space-y-6">
              <div className="w-full flex justify-between items-center">
                <h3 className="md:text-2xl text-xl font-semibold text-gray-800">Addresses</h3>
                <Link to="/profile/edit" className="flex items-center gap-x-1.5 text-base text-gray-600 border border-gray-300 px-3 py-1.5 rounded-4xl hover:shadow-sm hover:bg-gray-50 transition-all">
                  Edit <CiEdit size={22} className="font-semibold text-gray-700" />
                </Link>
              </div>
              ...
              <div className="grid md:grid-cols-2 grid-cols-1 gap-6">
                <div className="space-y-1">
                  ...
                  <p className="text-base font-medium text-gray-800">
                    {street ? `Street ${street}, ${city}` : "No default address yet"}
                  </p>
                  <p className="text-sm text-gray-500">{country || ""}</p>
                  {!hasAddress && (
                    <Link to="/profile/edit" className="text-sm text-teal-600 hover:underline mt-2 inline-block">+ Add address</Link>
                  )}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-base font-medium text-gray-800">
                    {street ? `Street ${street}, ${city}` : "No billing address yet"}
                  </p>
                  <p className="text-sm text-gray-500">{country || ""}</p>
                </div>
              </div>
            ...
            {profileStatus === "failed" && (
              <ErrorMessage message={profileError || "Failed to load user profile"} />
            )}
